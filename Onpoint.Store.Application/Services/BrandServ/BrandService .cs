...
namespace Onpoint.Store.Application.Services.Brand
{
    public class BrandService : IBrandService
    {
        ...
        public async Task<ServiceResult<IEnumerable<ProductDto>>> GetProductsByBrandIdAsync(int brandId)
        {
            ...
            var products = await _unitOfWork.Brands.GetProductsByBrandIdAsync(brandId);
            var dtos = _mapper.Map<IEnumerable<ProductDto>>(products);

            return _resultHandler.Success(dtos);
        }
        public async Task<ServiceResult<IEnumerable<BrandDto>>> GetAllAsync()
        {
        ...
        public async Task<ServiceResult<BrandDto>> GetByIdAsync(int id)
        {
        ...
        {
            var (items, totalCount) = await _unitOfWork.Brands.GetFilteredPagedAsync(searchTerm, isActive, pageNumber, pageSize);
            var dtos = _mapper.Map<IReadOnlyList<BrandDto>>(items);

            return _resultHandler.Success((dtos, totalCount));
        }

        public async Task<ServiceResult<BrandDto>> CreateAsync(CreateBrandDto dto)
        {
            await _createValidator.ValidateAndThrowAsync(dto);

            var brand = _mapper.Map<Domin.Entities.Brand>(dto);
            brand.Slug = await GenerateUniqueSlugAsync(dto.Name, null);
        ...
        public async Task<ServiceResult<BrandDto>> UpdateAsync(int id, UpdateBrandDto dto)
        {
            ...
            {
                brand.Slug = await GenerateUniqueSlugAsync(dto.Name, id);
            }
        ...
        {
            var brand = await _unitOfWork.Brands.GetWithProductsAsync(id);
            if (brand == null)
                return _resultHandler.NotFound<bool>("Brand not found");
        ...
        private async Task<string> GenerateUniqueSlugAsync(string name, int? excludeBrandId)
        {
            var baseSlug = SlugHelper.GenerateSlug(name);
            var slug = baseSlug;
            var counter = 2;

            while (await _unitOfWork.Brands.SlugExistsAsync(slug, excludeBrandId))
            {
